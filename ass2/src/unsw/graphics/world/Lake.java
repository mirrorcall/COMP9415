...
public class Lake {
    ...
    private static final float WIDTH_OF_LAKE = 1;
    private static final float ALTITUDE_OF_LAKE = 0;
    ...
    private LineStrip3D curve;
    private LineStrip3D inCurve;
    private LineStrip3D exCurve;

    private TriangleMesh model;
    private boolean isFlip;
    private int frameCounter;       // determine how many fps is lake is running
     ...
     * @param points
     */
    public Lake(List<Point2D> points) {
        this.width = WIDTH_OF_LAKE;
        this.points = points;
        this.altitude = ALTITUDE_OF_LAKE;

        this.isFlip = true;
        this.frameCounter = 0;
    }

    public void updateTexture(GL3 gl) {
        if (frameCounter == FPS) {
            if (!isFlip) {
                modelTexture = new Texture(gl, "res/textures/lakeflip.jpg", "jpg", false);
                isFlip = !isFlip;
            } else {
                modelTexture = new Texture(gl, "res/textures/lake.jpg", "jpg", false);
                isFlip = !isFlip;
            }
            frameCounter = 0;
        }
    ...
    private void makeExtrusion() {
        ...
        // Set DEBUGGING to true in World.java to show the internal and external curve with LineStrip3D
        ...
        for(float t = 0f; t <= this.size(); t += dt) {
            ...
            curve.add(new Point3D(point(t).getX(), altitude, point(t).getY()));
            inCurve.add(inP);
            exCurve.add(exP);
