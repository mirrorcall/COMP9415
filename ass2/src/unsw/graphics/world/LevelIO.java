...
public class LevelIO {
    ...
    public static Terrain load(File mapFile) throws FileNotFoundException {
        ...
        // Add lakes forming a circle
        List<Point2D> spine1 = Arrays.asList(
                new Point2D(0.5f,0),
                new Point2D(0.5f, -MathUtil.distance(0.5f)),
                new Point2D(MathUtil.distance(0.5f), -0.5f),
                new Point2D(0, -0.5f)
        );
        List<Point2D> spine2 = Arrays.asList(
                new Point2D(0, -0.5f),
                new Point2D(-MathUtil.distance(0.5f), -0.5f),
                new Point2D(-0.5f, -MathUtil.distance(0.5f)),
                new Point2D(-0.5f, 0)
        );
        List<Point2D> spine3 = Arrays.asList(
                new Point2D(-0.5f, 0),
                new Point2D(-0.5f, MathUtil.distance(0.5f)),
                new Point2D(-MathUtil.distance(0.5f), 0.5f),
                new Point2D(0, 0.5f)
        );
        List<Point2D> spine4 = Arrays.asList(
                new Point2D(0, 0.5f),
                new Point2D(MathUtil.distance(0.5f), 0.5f),
                new Point2D(0.5f, MathUtil.distance(0.5f)),
                new Point2D(0.5f, 0)
        );
        terrain.addLake(spine1);
        terrain.addLake(spine2);
        terrain.addLake(spine3);
        terrain.addLake(spine4);
